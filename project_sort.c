...
static void QuickSort(double *a, int left, int right) {
    ...
    do { //выполняем действия, пока индексы i и j не пересеклись
        while (a[i] < comp && i < right) { //если элемент меньше компаранда, и он слева от указателя right, то все хорошо, переходим к следующему
            count_comp_2++; 
            i++;
        }
        //сейчас элемент a[i] больше компаранда, нам нужно найти элемент a[j], который меньше компаранда, но стоит правее, чем a[i] 

        // Если условие while не выполнилось, но было сравнение двух элементов массива
        if (i != (left + right) / 2 && (a[i] >= comp || i == right))
            count_comp_2++; 

        while (comp < a[j] && j > left) { //аналогично, если элемент справа от компаранда, и он справа от указателя left, то все хорошо, идем к следующему
            count_comp_2++; 
            j--;
        }
        ...
        // Если условие while не выполнилось, но было сравнение двух элементов массива      
        if (j != (left + right) / 2 && (a[j] <= comp || j == left))
            count_comp_2++;
...
int main() {
    ...
    for (int k = 0; k < 4; k++) {
        ...
        printf("Массив %d после сортировки методом Шелла:\n", k + 1);
        print_array(arrays[k], n);
        printf("число сравнений: %lld, число обменов: %lld\n", count_comp_1, count_swap_1);

        count_comp_2 = 0;
        count_swap_2 = 0;
        QuickSort(original, 0, n - 1); 
        printf("Массив %d после быстрой сортировки:\n", k + 1);
